...
const FEATURES_SEED: Record<string, ExistingFeature[]> = {
  ...
  '报价工作台': [
    { name: 'Stage1 功能清单', description: 'Stage1FeatureList.tsx：默认 18 项/7 模块，支持 hover 新增子功能、Excel 导入导出、版本快照。' },
    { name: 'Stage2 人天评估', description: 'Stage2EvalSheet.tsx：各岗位人天评估，方向键+Tab 连续填充，按模块/子功能快捷批量。' },
    { name: 'Stage3 销售报价', description: 'Stage3QuoteWizard.tsx：7 步向导，系统校验∑分项=总人天、∑成本=总报价。' },
    { name: 'Stage4 审批盖章', description: 'Stage4Approval.tsx：三人并行会签→待盖章→盖章→生成 PDF；驳回→退回→重审。' },
    { name: '报价状态机', description: 'quoteFlow.ts（53 单测）：草稿→已确认→评估完成→转派→汇总→审批中→盖章→发出；支持撤回/作废。' },
    { name: '角色切换器', description: 'RoleSwitcher：6 角色模拟（产品经理/技术负责人/销售部/决策层/董助/销售），各角色不同权限。' },
    { name: '报价中心', description: 'QuotationCenter.tsx：报价单列表，按状态/创建人筛选，版本管理，作废功能。' },
  ],
  '客户与报价基础': [
  ...
  '合同签约': [
    ...
    { name: '补充协议', description: '基于主合同创建补充协议，继承客户信息与审批流，独立版本管理。' },
...
const PLANNED_SEED: Record<string, string[]> = {
  ...
  '报价工作台': ['上浮比例配置（原始金额→上浮信息→最终金额）'],
  '线索全流程': ['签约后综合视图（销售在线索详情看项目执行）', '签约自动生成未确认项目'],
  '合同签约': ['报价成交生成主合同', '多份补充报价/补充合同列表', '需求变更闭环（补充报价→补充合同）'],
  '项目管理': ['未确认项目与管理员指派产品经理', '产品经理默认列表隐藏未确认/未指派', '售前历程交接包（跟进/会议/演示/资料只读）'],
  // 交付域
  '日报工时': ['跨月跨年日报（时间轴优化）'],
  ...
  '工作台与审批': ['审批模板（串行/并行/会签节点配置）', '数据看板（管理者：销售业绩/项目进度/财务概览）'],
  // 跨域工具
