...
const FEATURES_SEED: Record<string, Omit<ExistingFeature, 'alpha'>[]> = {
  ...
  '线索全流程': [
    ...
    { name: '线索跟进记录', description: '跟进记录同步线索状态和等级；明确的下次跟进承诺进入统一待办，并按业务键更新、完成与转交。' },
    ...
    { name: '线索治理', description: 'LeadGovernance 读取真实线索与系统规则，覆盖时效异常、重复合并和退回质检；所有处置人工确认，不做随机评分或自动处罚。' },
    { name: '售前群沟通分析', description: 'vite 中间件 wx-cli-bridge：wx CLI 导出企微售前群聊天记录，DeepSeek 做沟通总结。' },
  ],
  '报价工作台': [
    { name: 'Stage1 功能清单', description: '两级模块+子功能（数据流转可挂端/平台）。支持标准五列 Excel 导入预览确认与 XLSX 导出；提交评估前至少一行。' },
  ...
  '合同签约': [
    ...
    { name: '合同归档', description: '合同详情支持多份扫描件上传归档；本次上传文件可预览、下载、删除，种子演示文件无源时明确禁用下载。' },
    ...
    { name: '合同列表', description: '合同列表/看板支持关键词、状态和签订日期筛选；新增入口进入真实合同向导，行内可打开详情。' },
  ],
  '跟进助手': [
    { name: '跟进承诺待办', description: '线索下次跟进承诺写入统一 Todo，并按业务键更新、完成和随负责人转交。' },
    { name: '销售机会提醒', description: '联系人生日、合同到期和维护期到期写入统一 Reminder，支持稍后提醒和人工转待办。' },
  ],
  '报价与合规增强': [
    { name: '报价差异与补充影响', description: '重新报价与补充报价分别使用 previousQuoteId 和 parentQuoteId，展示独立差异与合同影响。' },
    { name: '报价责任协作', description: '支持销售/技术评估人转交和限时代理，校验重叠并保留记录。' },
    { name: 'α 签署演示与合规档案', description: '无电子签名法律效力的状态演示、真实线下证据与合规证据只读投影，支持 HTML/JSON 导出。' },
  ],
  // ========== 交付域 ==========
  '项目管理': [
  ...
  '差旅管理': [
    ...
    { name: '报销与借款', description: '出差详情页签支持新建报销/借款草稿、金额汇总和详情审批轨迹抽屉；借款保留报销冲抵记录。' },
  ...
  '多渠道线索接入': [
    ...
    { name: '渠道成本分析', description: 'LeadCostAnalysis 按日期与平台生成排名、对比图和雷达图，应用筛选后可导出完整 XLSX。' },
  ...
  '组织与权限': [
    ...
    { name: '系统通用配置', description: 'SystemConfig 的消息渠道、备份策略和业务参数可持久化保存，并支持 JSON 备份下载与校验恢复。' },
  ],
  '人资行政': [
...
export function createSeedBoard(): FeatureBoard {
  ...
  const ALL_MODULES: Array<{ module: string; domain: Domain; isPlanned: boolean; scope?: string; features: ExistingFeature[]; planned?: string[]; note: string }> = [
    ...
    { module: '报价与合规增强', domain: '销售域', isPlanned: false, features: FEATURES_SEED['报价与合规增强'] ?? [], planned: ['真实电子签名供应商（不纳入 α）'], note: 'α 完成 Diff、影响、转交/代理、签署演示与合规投影' },
    // === 交付域（优先级 2）===
